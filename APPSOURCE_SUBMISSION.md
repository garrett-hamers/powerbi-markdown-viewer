...
- Syntax highlighting honors validated `language-*` hints, falls back to bounded automatic detection only for absent or unknown hints, and parses the highlighter's controlled span grammar directly into a `DocumentFragment` without string-to-DOM sinks.
...
- The certification audit reports no external requests; application source and the generated PBIVIZ JavaScript resource are checked for executable `innerHTML`, `fetch`, `XMLHttpRequest`, `WebSocket`, `eval`, `Function`, and dynamic-import sinks while ignoring inert strings and comments.
...
- The prior `1.0.5.0` embedded payload hash no longer applies. Recompute and record the `1.0.7.0` embedded PBIVIZ metadata/content SHA-256 immediately before submission.
...
- The tracked `sample\SampleReport.pbix` is Microsoft Information Protection/RMS-protected and contains visual version `1.0.0.0`. Do not rewrite this binary with repository automation. Replace it manually in Power BI Desktop with an unprotected PBIX that works offline and contains exact visual version `1.0.7.0`; verify incoming filters, both context-menu modes, safe-link launching, every formatting property, keyboard focus, high contrast, resize/scroll behavior, save/reopen, and publish/export before upload.
...
- [ ] Replace the protected stale `sample\SampleReport.pbix` with an unprotected offline PBIX saved using exact visual version 1.0.7.0
