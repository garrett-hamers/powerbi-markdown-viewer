...
- Markdown is lexed into controlled DOM fragments using narrow tag/attribute allowlists; regression coverage proves legacy `background` URLs on tables/cells and other automatic resource attributes cannot survive without string-to-DOM sinks.
- Syntax highlighting uses an explicit core language set, honors validated `language-*` hints, bounds automatic detection, and parses the highlighter's controlled span grammar into a `DocumentFragment`.
...
- The certification audit reports no external requests; application source and the generated PBIVIZ JavaScript resource are checked for executable `innerHTML`, `fetch`, `XMLHttpRequest`, `WebSocket`, `eval`, `Function`, and dynamic-import sinks while ignoring inert strings and comments. There is no dependency allowlist waiver; dependency code is scanned as embedded.
